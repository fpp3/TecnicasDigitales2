...
\begin{itemize}
    \item \textbf{APB2ENR} (Offset: \texttt{0x18})
    \begin{itemize}
    ...
    \end{itemize}
    \item \textbf{APB1ENR} (Offset: \texttt{0x1C})
    \begin{itemize}
...
\begin{itemize}
    \item \textbf{AFIO\_EXTICR1 a 4} (Offsets: \texttt{0x08} a \texttt{0x14})
    \begin{itemize}
...
\begin{itemize}
    \item \textbf{EXTI\_IMR} (Offset: \texttt{0x00})
    \begin{itemize}
        \item \texttt{[19:0]} \texttt{MRy}: 1 = Habilita (desenmascara) int. $y$.
    \end{itemize}
    \item \textbf{EXTI\_RTSR / FTSR} (Offsets: \texttt{0x08} / \texttt{0x0C})
    \begin{itemize}
        \item \texttt{[19:0]} \texttt{TRy}: 1 = Flanco ascendente / descendente.
    \end{itemize}
    \item \textbf{EXTI\_PR} (Offset: \texttt{0x14})
    \begin{itemize}
...
\begin{itemize}
    \item \textbf{Offsets}: ISER \texttt{0x000}, ICER \texttt{0x080}, ISPR \texttt{0x100}, ICPR \texttt{0x180}
    ...
    \begin{itemize}
        \item \texttt{EXTI0} a \texttt{EXTI4}: 6 a 10
        \item \texttt{ADC1\_2}: 18
        \item \texttt{EXTI9\_5}: 23
        \item \texttt{TIM2}: 28
        \item \texttt{USART1}: 37 \textit{(Requiere ISER1)}
        \item \texttt{EXTI15\_10}: 40 \textit{(Requiere ISER1)}
    \end{itemize}
\end{itemize}

\section*{SysTick (\texttt{0xE000 E010})}
\begin{itemize}
    \item \textbf{CTRL} (Offset: \texttt{0x00})
    \begin{itemize}
        \item \texttt{[16]} \texttt{COUNTFLAG}, \texttt{[2]} \texttt{CLKSOURCE}, \texttt{[1]} \texttt{TICKINT}, \texttt{[0]} \texttt{ENABLE}
    \end{itemize}
    \item \textbf{LOAD / VAL / CALIB} (Offsets: \texttt{0x04} / \texttt{0x08} / \texttt{0x0C})
    \begin{itemize}
...
\begin{itemize}
    \item \textbf{CR2} (Offset: \texttt{0x08})
    \begin{itemize}
        \item \texttt{[23]} \texttt{TSVREFE}: 1 = Habilita sensor interno Temp.
        \item \texttt{[22]} \texttt{SWSTART}: 1 = Inicia lectura regular manual.
        \item \texttt{[20]} \texttt{EXTTRIG}: 1 = Habilita disparo externo.
        \item \texttt{[19:17]} \texttt{EXTSEL}: Elige el evento disparador.
        \item \texttt{[1]} \texttt{CONT}: 1 = Continuo, 0 = Single.
        \item \texttt{[0]} \texttt{ADON}: 1 = Enciende ADC / Dispara conversión.
    \end{itemize}
    \item \textbf{SR} (Offset: \texttt{0x00})
    \begin{itemize}
        \item \texttt{[1]} \texttt{EOC}: 1 = Dato listo. Se limpia al leer.
    \end{itemize}
    \item \textbf{SQR1} (Offset: \texttt{0x2C})
    \begin{itemize}
        \item \texttt{[23:20]} \texttt{L}: Cant. total de conversiones menos 1.
    \end{itemize}
    \item \textbf{SQR3} (Offset: \texttt{0x34})
    \begin{itemize}
        \item \texttt{[4:0]} \texttt{SQ1}: Número del 1er canal a leer.
    \end{itemize}
    \item \textbf{DR} (Offset: \texttt{0x4C})
    \begin{itemize}
        \item \texttt{[15:0]}: Dato convertido de lectura regular.
    \end{itemize}
\end{itemize}

\section*{USART1 (\texttt{0x4001 3800})}
\begin{itemize}
    \item \textbf{CR1} (Offset: \texttt{0x0C})
    \begin{itemize}
        \item \texttt{[13]} \texttt{UE}: 1 = Habilita USART.
        \item \texttt{[5]} \texttt{RXNEIE}: 1 = Int. de recepción habilitada.
        \item \texttt{[3]} \texttt{TE} / \texttt{[2]} \texttt{RE}: 1 = Habilita Transm. / Recep.
    \end{itemize}
    \item \textbf{BRR} (Offset: \texttt{0x08}) 
    \begin{itemize}
        \item \texttt{[15:4]} \texttt{DIV\_Mantissa} / \texttt{[3:0]} \texttt{DIV\_Fraction}.
    \end{itemize}
    \item \textbf{SR} (Offset: \texttt{0x00})
    \begin{itemize}
        \item \texttt{[7]} \texttt{TXE}: 1 = Buffer vacío. \texttt{[6]} \texttt{TC}: 1 = Transmisión compl.
        \item \texttt{[5]} \texttt{RXNE}: 1 = Dato recibido.
    \end{itemize}
    \item \textbf{DR} (Offset: \texttt{0x04})
    \begin{itemize}
        \item \texttt{[8:0]}: Dato a enviar o recién recibido.
    \end{itemize}
\end{itemize}

\section*{TIM2 - PWM (\texttt{0x4000 0000})}
\begin{itemize}
    \item \textbf{CR1} (Offset: \texttt{0x00})
    \begin{itemize}
        \item \texttt{[0]} \texttt{CEN}: 1 = Enciende timer.
    \end{itemize}
    \item \textbf{PSC / ARR} (Offsets: \texttt{0x28} / \texttt{0x2C})
    \begin{itemize}
        \item \texttt{[15:0]}: Divisor de reloj / Tope (Período PWM).
    \end{itemize}
    \item \textbf{CCMR1 / CCMR2} (Offsets: \texttt{0x18} / \texttt{0x1C})
    \begin{itemize}
        \item \texttt{[6:4] OC1M} / \texttt{[14:12] OC2M}: Modos (ej. 110 PWM).
        \item \texttt{[3] OC1PE} / \texttt{[11] OC2PE}: 1 = Preload enable.
    \end{itemize}
    \item \textbf{CCER} (Offset: \texttt{0x20})
    \begin{itemize}
        \item \texttt{[1]} \texttt{CC1P}: 1 = Salida invertida. \texttt{[0]} \texttt{CC1E}: 1 = Habilita pin.
    \end{itemize}
    \item \textbf{CCRx} (Offsets: \texttt{0x34} a \texttt{0x40})
    \begin{itemize}
        \item \texttt{[15:0]}: Umbral de comparación (Duty Cycle).
    \end{itemize}
\end{itemize}
